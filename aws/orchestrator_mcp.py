...
mcp_config = McpServerJsonConfiguration('./orchestrator_mcp.json')
log = get_mcp_client_logger()

URI_SCHEME = 'orchestrator-mcp'

mcp = FastMCP(
    name="OrchestratorMcp",
    instructions="""
    Provides information about Orchestrator.
    """)
...
def main_fastapi(mcp:FastMCP, mcp_config: McpServerJsonConfiguration):
    ...
    # Create FastAPI app
    app = FastAPI(
        title="Orchestrator MCP Service",
        description="A service that data about Orchestrator",
        version="1.0.0",
    ...
    async def root() -> dict[str, str]:
        ...
        return {
            "service": "Orchestrator MCP Service",
            "version": "1.0.0",
            "status": "running",
        }
